    ...
    for item in output_info:
        if item[1] == 'tanh':
            ...
            if item[0] != 2:
                ...
                data_t.append(1 - F.sigmoid(data[:, st:st+1]))
                data_t.append(F.sigmoid(data[:, st:st+1]))
            st = ed
        else:
            assert 0
    ...
    for item in output_info:
        if item[1] == 'tanh':
            ...
            if item[0] != 2:
                ...
                tmp = F.binary_cross_entropy_with_logits(data[:, st], torch.argmax(c[:, st_c:ed_c], dim=1).float(), reduction='none')
            loss.append(tmp)
            st = ed
            st_c = ed_c
        else:
...
class BGMWGAN2Synthesizer(SynthesizerBase):
