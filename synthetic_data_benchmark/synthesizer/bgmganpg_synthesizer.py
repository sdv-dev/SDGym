...
class Generator(nn.Module):
    ...
    def forward(self, input):
        data = self.seq(input)
        return data, self.sigma

def apply_sampling(data, sigma, output_info):
    data_t = []
    st = 0
    for item in output_info:
        if item[1] == 'tanh':
            ed = st + item[0]
            rand = torch.randn_like(data[:, st:ed])
            data_t.append(rand * sigma[st] + torch.tanh(data[:, st:ed]))
            st = ed
        elif item[1] == 'softmax':
            ed = st + item[0]
            rand = torch.multinomial(torch.softmax(data[:, st:ed], dim=-1), 1)
            tmp = torch.zeros_like(data[:, st:ed])
            tmp[torch.arange(data.size()[0]), rand.view(-1)] = 1
            data_t.append(tmp)
            st = ed
        else:
            assert 0
...
def compute_policy_grad(data, sigma, sample, output_info, reward):
    loss = 0
    st = 0
    for item in output_info:
        if item[1] == 'tanh':
            ed = st + item[0]
            std = sigma[st]
            loss = loss + ((sample[:, st] - torch.tanh(data[:, st])) ** 2 / 2 / (std ** 2)) + torch.log(std)
            st = ed
        elif item[1] == 'softmax':
            ed = st + item[0]
            loss = loss + F.cross_entropy(data[:, st:ed], torch.argmax(sample[:, st:ed], dim=-1), reduction='none')
            st = ed
        else:
            assert 0
    ...
    def train(self, train_data):
        ...
        for i in range(max_epoch):
            for id_ in range(steps_per_epoch):
                ...
                fake, sigma = generator(torch.cat([fakez, c1], dim=1))
                fakeact = apply_sampling(fake, sigma, self.transformer.output_info)
                ...
                loss_g = compute_policy_grad(fake, sigma, fakeact, self.transformer.output_info, torch.sigmoid(y_fake) - torch.sigmoid(0.5 * (y_real.mean() + y_fake.mean()))) + cross_entropy
                ...
                generator.sigma.data.clamp_(0.01, 1.)
