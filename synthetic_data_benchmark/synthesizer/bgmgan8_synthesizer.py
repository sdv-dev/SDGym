...
from .synthesizer_utils import NoisyBGMTransformer, CONTINUOUS, ORDINAL, CATEGORICAL
import numpy as np
import torch
...
class BGMGAN8Synthesizer(SynthesizerBase):
    ...
    def train(self, train_data):
        ...
        self.transformer = NoisyBGMTransformer(self.meta)
        ...
        for i in range(max_epoch):
            ...
            fake_images = fakeact[:32, :-10]
            fake_images = fake_images.view(fake_images.size(0), 1, 12, 12, 2)
            fake_images = fake_images[:, :, :, :, 1]
            save_image(fake_images.data, os.path.join("tmp", 'fake_images-{}.png'.format(i+1)))
