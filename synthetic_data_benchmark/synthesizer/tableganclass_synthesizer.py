...
class TableganClassTransformer(object):
...
class Classifier(nn.Module):
    def __init__(self, meta, side, layers, test_index):
    ...
    def forward(self, input_):
        input_[self.test_index] = torch.zeros(input_[self.test_index].shape)
        input_concat = torch.cat(input_, dim=1)
        input_padded = F.pad(input_concat, (0, self.side * self.side - input_concat.shape[1]))
        input_square = input_padded.reshape((input_padded.shape[0], 1, self.side, self.side))
        return self.seq(input_square)
...
def determine_layers(side, randomDim, numChannels):
    ...
    layers_D = []
    layers_C = []
    for layer in [layers_D, layers_C]:
        for prev, curr in zip(layer_dims, layer_dims[1:]):
            layer += [nn.Conv2d(prev[0], curr[0], 4, 2, 1, bias=False),
                      nn.BatchNorm2d(curr[0]),
                      nn.LeakyReLU(0.2, inplace=True)]
        layer += [nn.Conv2d(layer_dims[-1][0], 1, 4, 2, 1, bias=False),
                  nn.Sigmoid()]
...
def get_label_function(item, meta, test_index):
    if meta[test_index]['type'] == CONTINUOUS:
        median = torch.median(item)
        return lambda i: (i.reshape((-1, 1, 1, 1)) > median).float()
    else:
        max_index = torch.argmax(torch.sum(item, dim=0))
        return (lambda i: i[:, max_index]).reshape((-1, 1, 1, 1))


# In[28]:


class TableganClassSynthesizer(SynthesizerBase):
    """docstring for TableganClassSynthesizer??"""
    ...
    def train(self, train_data):
        self.transformer = TableganClassTransformer(self.meta)
        ...
        classifier = Classifier(self.meta, self.side, layers_C, self.test_index).to(self.device)
        ...
        optimizerC = optim.Adam(classifier.parameters(), lr=1e-4, betas=(0.5, 0.9), eps=1e-3, weight_decay=self.l2scale)
        ...
        get_labels = get_label_function(train_data[self.test_index], self.meta, self.test_index)
        ...
        for i in range(max_epoch):
            for id_, data in enumerate(loader):
                ...
                labels = get_labels(real[self.test_index]).to(self.device)
                y_class = classifier(real)
                loss_c = nn.BCELoss()(y_class, labels)
                loss_c.backward()
                optimizerC.step()
                ...
                labels = get_labels(fake[self.test_index]).to(self.device)
                y_fake = discriminator(fake)
                y_class = classifier(real)
                loss_g = -(torch.log(y_fake + 1e-12).mean()) + nn.BCELoss()(y_class, labels)
                loss_g.backward()
                optimizerG.step()

                if((id_+1) % 50 == 0):
                    print(i+1, id_+1, loss_d, loss_g, loss_c)
                    break

            print(i+1, loss_d, loss_g, loss_c)
...
if __name__ == "__main__":
    run(TableganClassSynthesizer())


# In[34]:


#get_ipython().system('jupyter nbconvert --to script tablegan_class_synthesizer.ipynb')
