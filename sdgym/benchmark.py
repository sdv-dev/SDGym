...
def _write_run_id_file(output_destination, synthesizers, job_args_list, s3_client=None):
    ...
    if s3_client:
        ...
        with open(f'{output_destination}/{file_name}', 'w') as file:
            file.write(file_contents)

    return run_id


def _update_run_id_file(output_destination, run_id, s3_client=None):
    file_name = Path(output_destination) / f'run_{run_id}.yaml'
    completed_date = datetime.today().strftime('%m_%d_%Y %H:%M:%S')
    if s3_client:
        bucket, key = _parse_s3_uri(output_destination)
        key_prefix = key.rstrip('/')
        key = f'{key_prefix}/run_{run_id}.yaml'
